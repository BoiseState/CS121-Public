	...
	private static void testGetPostString() {
		...
		try {
			...
			try {
				...
				if (!lines[0].contains(ADA_POST_AUTHOR)) {
					testResults += subTestFailure(subtest, "first line contains post author " + ADA_POST_AUTHOR, lines[0]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[0].contains(ADA_POST_TEXT)) {
					testResults += subTestFailure(subtest, "first line contains post text \"" + ADA_POST_TEXT + "\"", lines[0]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[1].contains(ADA_POST_COMMENT1_AUTHOR)) {
					testResults += subTestFailure(subtest, "second line contains comment author " + ADA_POST_COMMENT1_AUTHOR, lines[1]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[1].contains(ADA_POST_COMMENT1_TEXT)) {
					testResults += subTestFailure(subtest, "second line contains comment text \"" + ADA_POST_COMMENT1_TEXT + "\"", lines[1]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[2].contains(ADA_POST_COMMENT2_AUTHOR)) {
					testResults += subTestFailure(subtest, "third line contains comment author " + ADA_POST_COMMENT2_AUTHOR, lines[2]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[2].contains(ADA_POST_COMMENT2_TEXT)) {
					testResults += subTestFailure(subtest, "third line contains comment text \"" + ADA_POST_COMMENT2_TEXT + "\"", lines[2]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (subtestPassed) {
			...
			try {
				...
				if (!lines[0].contains(KATHERINE_POST_AUTHOR)) {
					testResults += subTestFailure(subtest, "first line contains post author " + KATHERINE_POST_AUTHOR, lines[0]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[0].contains(KATHERINE_POST_TEXT)) {
					testResults += subTestFailure(subtest, "first line contains post text \"" + KATHERINE_POST_TEXT + "\"", lines[0]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (subtestPassed) {
			...
			try {
				...
				if (!lines[0].contains(GRACE_POST_AUTHOR)) {
					testResults += subTestFailure(subtest, "first line contains post author " + GRACE_POST_AUTHOR, lines[0]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[0].contains(GRACE_POST_TEXT)) {
					testResults += subTestFailure(subtest, "first line contains post text \"" + GRACE_POST_TEXT + "\"", lines[0]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[1].contains(GRACE_POST_COMMENT1_AUTHOR)) {
					testResults += subTestFailure(subtest, "second line contains comment author " + GRACE_POST_COMMENT1_AUTHOR, lines[1]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (!lines[1].contains(GRACE_POST_COMMENT1_TEXT)) {
					testResults += subTestFailure(subtest, "second line contains comment text \"" + GRACE_POST_COMMENT1_TEXT + "\"", lines[1]);
					testPassed = false;		
					subtestPassed = false;					
				}
				if (subtestPassed) {
